...
class Dir(GenericFile.File):
    ...
    @property
    def logs(self, **kwargs) -> Generator:#[LogFile.Log]:
        """A list of LogFile.Log instances found in the directory."""
        pool = Pool()
        yield from pool.execute(LogFile.Log, self.file_objects, progress_bar=True, **kwargs)
        # return [LogFile.Log(l.path, **kwargs) for l in self if not l.is_dir()]
